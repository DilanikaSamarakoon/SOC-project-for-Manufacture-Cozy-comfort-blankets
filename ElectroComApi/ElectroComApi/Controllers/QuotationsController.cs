...
namespace ElectroComApi.Controllers
{
    ...
    public class QuotationsController : ControllerBase
    {
        ...
        public QuotationsController(ElectroComDbContext context)
        {
            _context = context;
        }

        /// <summary>
        /// Handles requests for product quotations from The GadgetHub for ElectroCom.
        ...
        public async Task<ActionResult<QuotationResponse>> RequestQuotation([FromBody] QuotationRequest request)
        {
            ...
            foreach (var productRequest in request.Products)
            {
                ...
                {
                    // --- Simulate Pricing and Availability Logic for ElectroCom ---
                    // Calculate price after discount
                    decimal pricePerUnit = productInDb.Price * (1 - productInDb.DiscountPercentage); // Now using 'Price'
                    pricePerUnit *= 1.03m; // ElectroCom's additional 3% markup
                    ...
                    {
                        estimatedDeliveryDays += 3; // Longer delivery for backorder/partial fulfillment
                    }
                ...
                {
                    // Product not found in ElectroCom's inventory
                    response.ProductQuotes.Add(new ProductQuote
                    {
                        ProductId = productRequest.ProductId,
                        PricePerUnit = 0m,
