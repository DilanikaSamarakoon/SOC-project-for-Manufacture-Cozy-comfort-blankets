...
namespace TechWorldApi.Controllers
{
    ...
    public class OrdersController : ControllerBase
    {
        ...
        public OrdersController(TechWorldDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<ActionResult<OrderConfirmation>> PlaceOrder([FromBody] OrderRequest request)
        {
            ...
            if (productInDb == null)
            {
                return NotFound(new OrderConfirmation
                {
                    ...
                    EstimatedDeliveryDays = -1,
                    Success = false,
                    Message = $"Product {request.ProductId} not found in TechWorld inventory."
                });
            }
            ...
            // --- Calculate Final Price and Delivery (re-confirming logic) ---
            decimal finalPricePerUnit = productInDb.Price * (1 - productInDb.DiscountPercentage);
            finalPricePerUnit *= 1.05m; // TechWorld's additional 5% markup
            ...
            var distributorOrderId = $"TW-ORDER-{Guid.NewGuid().ToString().Substring(0, 8).ToUpper()}"; // Generate a unique order ID
            ...
            return Ok(new OrderConfirmation
            {
                ...
                Success = true,
                Message = $"Order for {request.Quantity} units of {request.ProductId} confirmed by TechWorld."
            });
        }
    }
}
